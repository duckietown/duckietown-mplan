...
class TrajectoryCreator(WorkerBase):
    ...
    def initIO(self):
        ...
        self.tile_current = self.findClosestTile(rospy.get_param(
            'x_pos_set') * self.tile_size, rospy.get_param('y_pos_set') * self.tile_size)
    ...
    def parseMapToTileList(self, map):
        ...
        for elem in map.markers:
            if elem.ns == 'tiles':
                ...
                if name != 'asphalt.dae':
                    quaternion = [elem.pose.orientation.x, elem.pose.orientation.y,
                    ...
                    self.tiles.append({'position': [
                                      elem.pose.position.x, elem.pose.position.y, yaw], 'type': name, 'entry_angle': yaw})
    ...
    def getNextTile(self, tile):
        ...
        xy = tile['position'] + \
            self.getUnitVecFromTheta(next_entry_angle, self.tile_size)
    ...
    def advance(self, Ts=1.0):
        ...
        cost_grid_origin = self.getCostGridOrigin(
        ...
        e = self.getUnitVecFromTheta(
            self.tile_current['entry_angle'] + math.pi / 2)[:2]
