...
class TrajectorySampler(WorkerBase):
    ...
    def initIO(self):
        ...
        self.trajectory_sub = rospy.Subscriber(
            'obst_avoid/trajectory', oamsg.TimedPath, self.trajectoryCb)
        ...
        self.command_pub = rospy.Publisher(
            'obst_avoid/twist', oamsg.Twist2DStamped, queue_size=1)

        self.cost_grid_viz_pub = rospy.Publisher(
            'obst_avoid/cost_grid', MarkerArray, queue_size=1)
    ...
    def markerCb(self, data):
        ...
        # mark that new trajectory has been calculated
        self.iterator = 0
    ...
    def advance(self, Ts=1.0):
        ...
        self.iterator = (self.iterator + 1)%int(round(self.cost_grid_params.get('n_t')*self.cost_grid_params.get('dt')/Ts))
        ...
        try:
            self.cost_grid_viz_pub.publish(self.cost_grid_marker.markers[k*marker_array_length:(k+1)*marker_array_length])
        except:
            pass

        # target position on trajectory to be reached after target_time
        x_set, y_set = self.trajectory.getPositionFromTimePoint(
            rospy.Time.now() + rospy.Duration(self.target_time))

        # position on trajectory, where actor should be now
        x_set_now, y_set_now = self.trajectory.getPositionFromTimePoint(
            rospy.Time.now())
        ...
        # publish
        command_msg = oamsg.Twist2DStamped()  # TODO, add proper message and populate it
        command_msg.header.stamp = rospy.Time.now()
        # either calculated value or vel_max is published
        command_msg.v = min([vel_set, self.vel_max])
        # either calculated value or omega_max is published
        command_msg.omega = min([omega_set, self.omega_max])
