...
def fleetPlannerCb(msg):
    ...
    for duck in msg.duckie_states:
        if duck.duckie_id.data == actor_duckiebot_id:
            ...
            actor_msg.moving_object.twist.x = math.cos(
                theta) * lx - math.sin(theta) * ly
            actor_msg.moving_object.twist.y = math.sin(
                theta) * lx + math.cos(theta) * ly
    ...
    obstacle_list_msg = oamsg.Obstacles()
...
def main():
    ...
    command_sub = rospy.Subscriber(
        '/obst_avoid/twist', oamsg.Twist2DStamped, commandCb)
