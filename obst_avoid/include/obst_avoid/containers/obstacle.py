...
class Obstacle:
    ...
    def init_obstacles_fun(self):
        ...
        obstacle_cost_fun = 2 * max_cost * 2**(-((((x + t * obs_x_dot - obs_x)**2 + (
            y + t * obs_y_dot - obs_y)**2) / radius**2)**(function_degree * radius / 2)))

        self.getCost = sp.lambdify(
            [x, y, t, obs_x, obs_y, obs_x_dot, obs_y_dot, radius], obstacle_cost_fun)
