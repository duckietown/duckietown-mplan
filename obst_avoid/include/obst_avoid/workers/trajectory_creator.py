...
class TrajectoryCreator(WorkerBase):
    ...
    def getNextTile(self, tile):
        ...
        if tile['type']=='straight.dae':
            ...
            next_entry_angle = tile['entry_angle'] + randint(-1,1)*math.pi/2*0

        elif tile['type']=='3way_left.dae':
            if delta_theta == 0:
                next_entry_angle = tile['entry_angle'] + randint(0,1)*math.pi/2*0
            if delta_theta == math.pi:
                next_entry_angle = tile['entry_angle'] + randint(-1,0)*math.pi/2*0
            if delta_theta == math.pi*3/2:
                next_entry_angle = tile['entry_angle'] + (randint(0,1)-1.0/2)*math.pi*0

        elif tile['type']=='3way_right.dae':
            if delta_theta == 0:
                next_entry_angle = tile['entry_angle'] + randint(-1,0)*math.pi/2
            if delta_theta == math.pi/2:
                next_entry_angle = tile['entry_angle'] + (randint(0,1)-1/2)*math.pi
            if delta_theta == math.pi:
                next_entry_angle = tile['entry_angle'] + randint(0,1)*math.pi/2
        else:
