...
from obst_avoid.workers.worker_base import WorkerBase
...
class PathViz(WorkerBase):
    ...
    def initIO(self):
        ...
        self.path_pub = rospy.Publisher(
            'obst_avoid_viz/path', Path, queue_size=10)
    ...
    def trajectoryCb(self, data):
        ...
        for i, elem in enumerate(data.positions):
            pose = PoseStamped()
            pose.header.stamp = data.start_time + \
                rospy.Duration(data.times[i] * data.duration)
